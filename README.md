# 냉집사 : AI로 식재료 인식 및 레시피 추천 앱

<br/><br/>

<p align="center">
  <img src="docs/assets/poster.jpg" width="80%"/>
</p>

<p align="center">
  <a href="https://kookmin-sw.github.io/2026-capstone-30/#about">냉집사 사이트 방문하기</a>
</p>

<br/><br/>

---

## 🔗 목차

#### 1️⃣ [💡 프로젝트 소개](#-프로젝트-소개)

#### 2️⃣ [🚀 주요 기능](#-주요-기능)

#### 3️⃣ [🎬 소개 영상](#-소개-영상)

#### 4️⃣ [👋 팀원 소개](#-팀원-소개)

#### 5️⃣ [🌐 시스템 구조](#-시스템-구조)

#### 6️⃣ [⚒️ 기술 스택](#️-기술-스택)

#### 7️⃣ [📖 사용법 (환경 설정)](#-사용법-환경-설정)

#### 8️⃣ [📂 폴더 구조](#-폴더-구조)

#### 9️⃣ [📝 참고 자료](#-참고-자료)

---

## 💡 프로젝트 소개

### 이제 냉장고는 '정리하는' 것이 아니라 '활용하는' 것입니다

- 냉집사는 냉장고 사진 한 장으로 AI가 식재료를 즉시 인식하고, 지금 바로 만들 수 있는 맞춤형 레시피를 추천해 드립니다.
- 식재료를 일일이 입력할 필요 없이 카메라 촬영만으로 냉장고 재고를 파악하고, 유튜브 영상·쇼핑 링크까지 한 번에 제공합니다.
- 식재료 인식부터 레시피 추천, 영상 가이드, 부족 재료 구매까지 식생활 전 과정을 하나의 AI 기반 앱에 담았습니다.

<br/>

<div align="center">

<table>
  <tr>
    <td width="20%" valign="top">
      <div align="center">
        <img src="https://img.icons8.com/ios/60/2A7D52/camera.png"/>
        <br/><br/>
        <b>AI 식재료 인식</b>
        <br/>
        <sub>냉장고 사진 한 장으로<br/>재료를 자동 분석해 목록 작성</sub>
      </div>
    </td>
    <td width="20%" valign="top">
      <div align="center">
        <img src="https://img.icons8.com/ios/60/2A7D52/chef-hat.png"/>
        <br/><br/>
        <b>맞춤 레시피 추천</b>
        <br/>
        <sub>보유 재료 기반으로<br/>지금 바로 만들 수 있는 레시피 매칭</sub>
      </div>
    </td>
    <td width="20%" valign="top">
      <div align="center">
        <img src="https://img.icons8.com/ios/60/2A7D52/chat.png"/>
        <br/><br/>
        <b>AI 챗봇</b>
        <br/>
        <sub>레시피 관련 궁금한 점을<br/>AI에게 바로 질문하고 답변</sub>
      </div>
    </td>
    <td width="20%" valign="top">
      <div align="center">
        <img src="https://img.icons8.com/ios/60/2A7D52/youtube-play.png"/>
        <br/><br/>
        <b>유튜브 영상 연동</b>
        <br/>
        <sub>레시피에 맞는<br/>신뢰도 높은 요리 영상 연결</sub>
      </div>
    </td>
    <td width="20%" valign="top">
      <div align="center">
        <img src="https://img.icons8.com/ios/60/2A7D52/shopping-cart.png"/>
        <br/><br/>
        <b>부족 재료 쇼핑 연동</b>
        <br/>
        <sub>레시피에 부족한 재료를<br/>즉시 구매 가능한 링크 제공</sub>
      </div>
    </td>
  </tr>
</table>

</div>

<br/>

### 추진 배경 및 필요성

현대 사회에서 1인 가구 및 맞벌이 가구의 증가로 인해 집에서 요리하는 빈도가 불규칙해지면서, 냉장고에 방치된 식재료가 유통기한을 넘겨 음식물 쓰레기로 발생하는 일이 지속적으로 나타나고 있습니다. 이는 개인의 경제적 손실로도, 환경 오염 측면에서도 개선이 필요한 문제로 인식되고 있습니다.

연구에 따르면 음식물 쓰레기의 상당량이 최종 유통 및 가정 내 소비 단계에서 발생하며, 특히 가정 부문에서의 발생 비중이 높은 것으로 보고됩니다. 이에 따라 가정 내 음식물 쓰레기 저감을 위한 체계적인 관리 방안이 필요합니다.

또한, 최근 급격한 외식 물가 상승, 요리 예능 등으로 인해 직접 요리를 해 먹으려는 '홈 쿡' 수요가 늘어나고 있으나, 요리에 익숙하지 않은 초보자들은 한번 요리에 사용하고 남은 재료, 혹은 냉장고에 쌓여가는 수많은 식재료들을 어떻게 활용해야 할지 몰라 매번 새로운 식재료를 구매하거나 배달 음식에 의존하는 악순환을 겪고 있습니다.

따라서 사용자가 현재 보유하고 있는 재료의 활용도를 극대화하고, 누구나 쉽고 간편하게 요리에 접근할 수 있도록 돕는 지능형 보조 시스템의 필요성이 매우 높아진 상황입니다.

---

## 🚀 주요 기능

### 📸 냉장고 식재료 인식 및 관리

- 스마트폰 카메라 촬영 또는 갤러리 다중 업로드 시 AI가 이미지를 분석해 식재료 자동 추출
- 7개 카테고리(양념·고기·채소·해산물·유제품·과일·기타)로 자동 분류 및 등록일 기준 정렬
- 수동 편집(추가/삭제) 및 냉장고 한번에 비우기 기능 지원

### 🍳 맞춤형 레시피 추천 및 영상 가이드

- 인식된 식재료 기반 즉시 조리 가능한 레시피 3~5개 자동 매칭
- 유튜브 인앱 플레이어로 신뢰도 높은 요리 영상을 외부 이동 없이 바로 재생
- 보유 재료와 부족 재료를 구분하고, 필요한 식재료 추천

### 👨‍🍳 단계별 요리 가이드 

- AI가 5~9단계로 분해한 조리 과정을 카드 형식으로 단계별 안내
- 각 단계 진입 시 한국어 TTS로 제목과 설명을 자동 낭독, 재생/정지 토글 지원
- 단계별 팁과 다음 단계 미리보기 제공으로 따라하기 쉽도록 제작

### ⭐ 요리 별점 및 자동 저장

- 요리 완료 후 1~5점 별점과 한 줄 메모로 만족도 기록
- 5점 평가 시 다음에 또 사용할 수 있도록 해당 레시피 저장

### 🔄 대체 식재료 안내 기능

- 특정 재료 부재 시, 사용자 냉장고 재료 중에서 맛·식감·역할이 유사한 대체재 1개 제안 (예: 봄동 → 상추)
- 프로필에 따라서 알레르기·식이제한(채식/비건/할랄) 위반 재료는 대체 후보에서 자동 제외

### 🛒 부족 식재료 쇼핑 연동

- 선택한 레시피 조리에 필요한 부족 재료를 네이버 쇼핑 검색 결과로 즉시 연결
- 여러 레시피의 부족 재료를 쇼핑 탭에 모아 레시피별 그룹화 및 체크 관리

### 💬 AI 챗봇 '냉집사 AI'

- 홈 화면 플로팅 버튼으로 언제든 호출 가능한 요리 전문 챗봇 제공
- 음성 입력(한국어 STT) 지원 및 시간대·계절 기반 초기 추천 문구 자동 제시

### ⏰ 푸시 알림 및 오래된 재료 알림

- 카테고리별 보관 기한 기반으로 오래된 재료를 자동 감지하여 홈 화면 배너로 안내
- 매일 18시에 각 카테고리 별 저장 시간이 지난 식재료 알림
- 짝수 정각에는 추천하는 유행 레시피 푸시 알림을 전체 사용자에게 발송

### 👤 맞춤형 프로필 설정

- 알레르기 유발 식품(6종) 제외 및 식이 제한(채식/비건/할랄), 선호 요리 종류(한·중·양·일) 반영
- 비로그인 시에도 로컬에 저장되며, 로그인 시 서버와 자동 동기화
- 개인의 건강 상태와 취향을 고려한 맞춤형 레시피·대체재 추천 적용

### 🔖 레시피 저장 및 검색

- 마음에 든 레시피를 기기 내부에 저장하고 이름으로 검색·삭제 가능
- 저장된 레시피도 추천 레시피와 동일하게 영상·대체재·요리 가이드 이용 가능

---

## 🎬 소개 영상

https://github.com/user-attachments/assets/90b171f0-a3ff-426a-a89b-575c0a758a28

---

## 👋 팀원 소개

| 이름 | 역할 |
|------|------|
| **임수민** (조장) | 프로젝트 리더, Back-End (레시피 매칭 엔진, MySQL 데이터베이스 설계, FCM 푸시 알림 서버 구축) |
| **이지우** | Front-End, UI/UX, AI (Flutter 기반 크로스 플랫폼 앱 UI/UX 구현 및 Google Gemini API 연동) |

---

## 🌐 시스템 구조

<img src="docs/assets/architecture.png" width="80%"/>

---

## ⚒️ 기술 스택

| 분류 | 기술 |
|------|------|
| **Frontend** | Flutter (Dart) |
| **Backend** | Node.js, Express |
| **Database** | MySQL |
| **AI** | OpenRouter API → Google Gemini 2.5 Flash |
| **외부 연동** | 네이버 쇼핑 검색 (url_launcher) |
| **푸시 알림** | Firebase Cloud Messaging, node-cron |
| **개발 환경** | Android Studio, VS Code |
| **협업 도구** | Git, GitHub, Notion |

---

## 📖 사용법 (환경 설정)

#### 1. 저장소 복제

```bash
git clone https://github.com/kookmin-sw/2026-capstone-30
cd 2026-capstone-30
```

#### 2. DB 설정

MySQL에서 데이터베이스를 생성하고 스키마를 적용합니다:

```sql
CREATE DATABASE naengjibsa CHARACTER SET utf8mb4;
USE naengjibsa;
SOURCE server/schema.sql;
```

이후 `server/db.js`의 접속 정보를 본인 환경에 맞게 수정합니다 (`host`, `user`, `password`).

#### 3. API 키 및 Firebase 설정

`server/.env` 파일을 새로 만들어 아래 내용을 입력:

```
OPENROUTER_API_KEY=발급받은_키_입력
PORT=3000
```

Firebase 콘솔 → 프로젝트 설정 → 서비스 계정에서 비공개 키를 발급받아 `server/firebase-admin-key.json`으로 저장합니다.
키 파일이 없으면 서버는 정상 실행되지만 푸시 알림은 비활성화됩니다.

#### 4. 서버 실행

```bash
cd server
npm install
npm run dev
```

서버가 정상 실행되면 다음 메시지가 출력됩니다:
```
서버 실행 중: http://0.0.0.0:3000
```

#### 5. Flutter 앱 실행

서버를 켜둔 상태에서 새 터미널을 열고:

```bash
cd flutter_app
flutter pub get
flutter run --no-enable-impeller
```

> 에뮬레이터 사용 시 Impeller 렌더러 충돌이 발생할 수 있어 `--no-enable-impeller` 옵션을 권장합니다.

---

## 📂 폴더 구조

```
capstone_project_team30/
├── flutter_app/                       # Flutter 모바일 앱
│   ├── android/                       # Android 설정
│   ├── assets/                        # 앱 아이콘, 캐릭터 이미지 등
│   └── lib/
│       ├── main.dart                  # 메인 화면, 하단 네비게이션, FCM 초기화
│       ├── constants.dart             # 서버 URL, 테마 색상
│       ├── models/
│       │   ├── recipe.dart            # 레시피·유튜브·유행 데이터 클래스
│       │   └── user_profile.dart      # 알레르기, 식이제한, 선호 요리
│       ├── services/
│       │   ├── api_service.dart       # 백엔드 HTTP 호출
│       │   └── storage_service.dart   # SharedPreferences (저장 레시피, 프로필, 재료)
│       ├── screens/
│       │   ├── splash_screen.dart                # 앱 시작
│       │   ├── home_screen.dart                  # 사진 업로드 → 재료 분석 → 레시피 추천 + 유통기한 배너
│       │   ├── recipe_detail_screen.dart         # 추천 레시피 상세 (유튜브·대체재·요리 가이드)
│       │   ├── saved_screen.dart                 # 저장된 레시피 + 검색 + 유행 레시피 추천
│       │   ├── saved_recipe_detail_screen.dart   # 저장된 레시피 상세 화면
│       │   ├── shopping_screen.dart              # 부족 재료 → 네이버 쇼핑 연동
│       │   └── profile_screen.dart               # 사용자 취향 설정
│       └── widgets/
│           ├── login_form.dart                   # 로그인·회원가입 폼
│           ├── chatbot_sheet.dart                # AI 챗봇
│           ├── cooking_guide_sheet.dart          # 단계별 요리 가이드 + TTS
│           └── recipe_rating_sheet.dart          # 요리 별점 시트
├── server/                            # Node.js 백엔드 서버
│   ├── server.js                      # 메인 서버 (API 라우터, 스케줄러)
│   ├── db.js                          # MySQL 커넥션 풀
│   ├── config.js                      # 환경 변수 로드
│   ├── schema.sql                     # 데이터베이스 스키마
│   ├── curated_trends.json            # 유행 레시피 데이터
│   ├── package.json                   # 서버 의존성
│   ├── .env                           # API 키·포트 (사용자 생성)
│   └── firebase-admin-key.json        # Firebase 서비스 계정 키 (사용자 생성)
└── docs/                              # GitHub Pages 소개 페이지
    ├── index.html
    ├── style.css
    ├── script.js
    └── assets/                        # 소개 페이지용 이미지·포스터
```

---

## 📝 참고 자료

[노션 링크](https://www.notion.so/3364608f68978062affde98e98c3aedd)

---

© 2026 굿파트너 팀 (국민대학교 소프트웨어융합대학). 비상업적, 교육 및 연구 목적으로만 사용 허가.
